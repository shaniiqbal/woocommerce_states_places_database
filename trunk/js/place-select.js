jQuery( function($) {
 ...
 $( 'body' ).on( 'country_to_state_changing', function(e, country, $container) {
  ...
  $( 'body' ).on( '$billing_state', function(e, country, $container) {
    var $statebox = $container.find( '#billing_state, #shipping_state, #calc_shipping_state' );
    var state = $statebox.val();
    $( document.body ).trigger( 'state_changing', [country, state, $container ] );
  });
 ...
 /* var $container = "";
  var state = "";
  $( document.body ).trigger( 'state_changing', ["US", state, $container ] );*/
 

  $( 'body' ).on( 'change', '#billing_state, #calc_shipping_state', function() {
  ...
  function cityToSelect( $citybox, current_cities ) {
    ...
    var options = '';
    for( var index in current_cities[0] ) {
      if ( current_cities.hasOwnProperty( index ) ) {
		  for(var cityindex in current_cities[ index ]){
			var cityName = current_cities[0][ cityindex ];
       		 options = options + '<option value="' + cityName.toLowerCase() + '">' + cityName + '</option>';
		  }
...
function matchPeople(input) {
  var states_json = wc_state_select_params.states.replace( /&quot;/g, '"' );
...
jQuery("#billing_state").after("<select name='billing_state' id='billing_state' name='billing_state' autocomplete='address-level1'></select>");
	jQuery("input#billing_state").remove();
var select = document.getElementById("billing_state"); 
var states_json = wc_state_select_params.states.replace( /&quot;/g, '"' );
var options = $.parseJSON( states_json );
	//var options = ["1", "2", "3", "4", "5"]; 
